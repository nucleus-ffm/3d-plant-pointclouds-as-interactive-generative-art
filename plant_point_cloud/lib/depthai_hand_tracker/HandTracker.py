...
import plant_point_cloud.lib.depthai_hand_tracker.mediapipe_utils as mpu
from plant_point_cloud.lib.depthai_hand_tracker.FPS import FPS, now
...
class HandTracker:
    ...
    def __init__(
        ...
        if self.input_type == "rgb":
            ...
            if self.use_lm:
                self.q_lm_out = self.device.getOutputQueue(
                    name="lm_out", maxSize=2, blocking=False
                )
                self.q_lm_in = self.device.getInputQueue(name="lm_in")
            if self.xyz:
                self.q_spatial_data = self.device.getOutputQueue(
                    name="spatial_data_out", maxSize=4, blocking=False
                )
                self.q_spatial_config = self.device.getInputQueue(
                    "spatial_calc_config_in"
                )
        ...
        if use_handedness_average:
            ...
            self.handedness_avg = [
                mpu.HandednessAverage() for i in range(self.max_hands)
            ]
    ...
    def pd_postprocess(self, inference):
        ...
        ).reshape((self.nb_anchors, 18))  # 896x18
    ...
    def lm_postprocess(self, hand, inference):
        ...
        if hand.lm_score > self.lm_score_thresh:
            ...
            lm_raw = np.array(
                inference.getLayerFp16("Identity_dense/BiasAdd/Add")
            ).reshape(-1, 3)
            ...
            if self.use_world_landmarks:
                hand.world_landmarks = np.array(
                    inference.getLayerFp16("Identity_3_dense/BiasAdd/Add")
                ).reshape(-1, 3)
    ...
    def spatial_loc_roi_from_palm_center(self, hand):
        ...
        rect_center = dai.Point2f(
            int(hand.pd_box[0] * self.frame_size)
            + half_size
            - zone_size // 2
            + self.crop_w,
            int(hand.pd_box[1] * self.frame_size)
            + half_size
            - zone_size // 2
            - self.pad_h,
        )
    ...
    def spatial_loc_roi_from_wrist_landmark(self, hand):
        ...
        rect_center = dai.Point2f(
            *(
                hand.landmarks[0]
                - np.array((zone_size // 2 - self.crop_w, zone_size // 2 + self.pad_h))
            )
        )
        rect_size = dai.Size2f(zone_size, zone_size)
        return dai.Rect(rect_center, rect_size)
    ...
    def next_frame(self):
        ...
        if self.input_type == "rgb":
            ...
            video_frame = frame[
                self.crop_h : self.crop_h + self.frame_size,
                self.crop_w : self.crop_w + self.frame_size,
            ]
            ...
            if not self.use_previous_landmarks:
                ...
                frame_nn.setData(
                    to_planar(
                        square_frame, (self.pd_input_length, self.pd_input_length)
                    )
                )
        ...
        if self.use_previous_landmarks:
            ...
            if (
                not self.solo
                and self.nb_hands_in_previous_frame == 1
                and len(hands) <= 1
            ):
                self.hands = self.hands_from_landmarks
            else:
        ...
        if self.use_lm:
            ...
            if len(self.hands) == 2:
                dist_rect_centers = mpu.distance(
                    np.array(
                        (self.hands[0].rect_x_center_a, self.hands[0].rect_y_center_a)
                    ),
                    np.array(
                ...
                if dist_rect_centers < 5:
                    ...
                    if self.trace & 1:
                        print(
                            "!!! Removing one hand because too close to the other one"
                        )
            ...
            self.hands_from_landmarks = [
                mpu.hand_landmarks_to_rect(h) for h in self.hands
            ]
            ...
            if self.use_handedness_average:
                if (
                    not self.use_previous_landmarks
                    or self.nb_hands_in_previous_frame != nb_hands
                ):
                    for i in range(self.max_hands):
                        self.handedness_avg[i].reset()
                for i in range(nb_hands):
                    self.hands[i].handedness = self.handedness_avg[i].update(
                        self.hands[i].handedness
                    )
            ...
            if (
                not self.solo
                and nb_hands == 2
                and (self.hands[0].handedness - 0.5) * (self.hands[1].handedness - 0.5)
                > 0
            ):
