...
class Event:
    ...
    def print_line(self):
        print(
            f"{self.time.strftime('%H:%M:%S.%f')[:-3]} : {self.category} {self.name} [{self.pose}] - hand: {self.handedness} - trigger: {self.trigger} - callback: {self.callback}"
        )
...
class HandController:
    def __init__(self, caller_class, thread_killer: ThreadEvent, config={}):
        self.config = merge_config(DEFAULT_CONFIG, config)
        self.caller_class = caller_class
        self.thread_killer: ThreadEvent = thread_killer
        ...
        self.caller_globals = sys._getframe(
            1
        ).f_globals  # Or vars(sys.modules['__main__'])
    ...
    def start_renderer(self):
        """Start the renderer. Call [stop_renderer] to stop the camera again."""
        from .HandTrackerRenderer import HandTrackerRenderer

        self.renderer = HandTrackerRenderer(
            self.tracker, **self.config["renderer"]["args"]
        )
        self.use_renderer = True

    def stop_renderer(self):
        """Stop the renderer. Call [start_renderer] to start the renderer again."""
        if self.renderer is not None:
            print("Exit the renderer")
            self.renderer.exit()
        ...
        if "pose_actions" in self.config:
            for pa in self.config["pose_actions"]:
                ...
                if isinstance(pose, list):
                    ...
                    pa["pose"] = [pose]
                optional_args = {
                    k: pa.get(k, self.config["pose_params"][k]) for k in optional_keys
                }
    ...
    def generate_events(self, hands):
        ...
        for i, pa in enumerate(self.pose_actions):
            ...
            if (
                hand
                and hand.gesture
                and (hand.label == pa["hand"] or pa["hand"] == "any")
                and hand.gesture in pa["pose"]
            ):
                if trigger == "continuous":
                    ...
                    if not hist.triggered:
                        if hist.time != 0 and (
                            self.frame_nb - hist.frame_nb <= pa["max_missing_frames"]
                        ):
                            if hist.time and (
                                (
                                    hist.first_triggered
                                    and self.now - hist.time > pa["next_trigger_delay"]
                                )
                                or (
                                    not hist.first_triggered
                                    and self.now - hist.time > pa["first_trigger_delay"]
                                )
                            ):
                ...
                if (
                    hist.triggered
                    and self.frame_nb - hist.frame_nb > pa["max_missing_frames"]
                ):
                    hist.time = self.now
                    hist.triggered = False
                    hist.first_triggered = False
    ...
    def process_events(self, events):
        for e in events:
            if e.callback == "_DEFAULT_":
                ...
                self.caller_globals[e.callback](self.caller_class, e)

    def loop(self) -> None:
        """Main loop for detecting hand gestures.

        This is executed as soon as the thread_killer is not set.
        :return: None
        """
    ...
    def tick(self) -> None:
        """Calculate one tick. This needs to be called in a endless loop."""
        self.now = monotonic()
        frame, hands, bag = self.tracker.next_frame()
        if frame is None:
            return
    ...
    def destroy(self):
        """Exit the render and the tracker. This should be called if the tick update loop is done"""
        self.stop_renderer()
        self.tracker.exit()
