...
def hand_landmarks_to_rect(hand):
    ...
    x1, y1 = (
        0.25 * (lms_xy[id_index_mcp] + lms_xy[id_ring_mcp])
        + 0.5 * lms_xy[id_middle_mcp]
    )
...
def recognize_gesture(hand):
    ...
    angle0 = angle(
        hand.norm_landmarks[0], hand.norm_landmarks[1], hand.norm_landmarks[2]
    )
    angle1 = angle(
        hand.norm_landmarks[1], hand.norm_landmarks[2], hand.norm_landmarks[3]
    )
    angle2 = angle(
        hand.norm_landmarks[2], hand.norm_landmarks[3], hand.norm_landmarks[4]
    )
    ...
    if (
        hand.norm_landmarks[8][1]
        < hand.norm_landmarks[7][1]
        < hand.norm_landmarks[6][1]
    ):
        hand.index_state = 1
    ...
    if (
        hand.norm_landmarks[12][1]
        < hand.norm_landmarks[11][1]
        < hand.norm_landmarks[10][1]
    ):
        hand.middle_state = 1
    ...
    if (
        hand.norm_landmarks[16][1]
        < hand.norm_landmarks[15][1]
        < hand.norm_landmarks[14][1]
    ):
        hand.ring_state = 1
    ...
    if (
        hand.norm_landmarks[20][1]
        < hand.norm_landmarks[19][1]
        < hand.norm_landmarks[18][1]
    ):
        hand.little_state = 1
...
class Body:
    def __init__(
        ...
        self.keypoints = (
            ...
            + self.keypoints_norm * self.crop_region.size
        ).astype(np.int32)
    ...
    def distance_to_wrist(self, hand, wrist_handedness, pad_w=0, pad_h=0):
        ...
        """
        return distance(
            hand.landmarks[0] - np.array([pad_w, pad_h]),
...
class BodyPreFocusing:
    ...
    def torso_visible(self, scores):
        ...
        """
        return (
            scores[BODY_KP["left_hip"]] > self.score_thresh
            or scores[BODY_KP["right_hip"]] > self.score_thresh
        ) and (
            scores[BODY_KP["left_shoulder"]] > self.score_thresh
    ...
    def determine_crop_region(self, body):
        ...
        if self.torso_visible(body.scores):
            center_x = (
                body.keypoints[BODY_KP["left_hip"]][0]
                + body.keypoints[BODY_KP["right_hip"]][0]
            ) // 2
            center_y = (
                body.keypoints[BODY_KP["left_hip"]][1]
                + body.keypoints[BODY_KP["right_hip"]][1]
            ) // 2
    ...
    def estimate_focus_zone_size(self, body):
        ...
        for s in segments:
            if (
                body.scores[BODY_KP[s[0]]] > self.score_thresh
        ...
        if lengths:
            if (
                body.scores[BODY_KP["left_hip"]] < self.score_thresh
    ...
    def get_focus_zone(self, body):
        ...
        def get_one_hand_zone(hand_label):
            ...
            if self.hands_up_only:
                ...
                if (
                    body.scores[BODY_KP[elbow_kp]] > self.score_thresh
                    and body.keypoints[BODY_KP[elbow_kp]][1]
                    < body.keypoints[BODY_KP[wrist_kp]][1]
                ):
                    return None
            ...
            if size == 0:
                return [
                    -self.pad_w,
                    -self.pad_h,
                    self.frame_size - self.pad_w,
                    self.frame_size - self.pad_h,
        ...
        if self.mode == "group":
            ...
            if body.scores[BODY_KP["left_wrist"]] > self.score_thresh:
                if body.scores[BODY_KP["right_wrist"]] > self.score_thresh:
                    if (
                        body.keypoints[BODY_KP["left_wrist"]][1]
                        > body.keypoints[BODY_KP["right_wrist"]][1]
                    ):
                        hand_label = "right"
                    else:
                        hand_label = "left"
