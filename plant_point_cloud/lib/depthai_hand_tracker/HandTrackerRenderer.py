...
class HandTrackerRenderer:
    ...
    def draw_hand(self, hand):
        if self.tracker.use_lm:
            ...
            if hand.lm_score > self.tracker.lm_score_thresh:
                ...
                if self.show_landmarks:
                    lines = [
                        np.array([hand.landmarks[point] for point in line]).astype(
                            np.int32
                        )
                    ...
                    if self.tracker.use_gesture:
                        ...
                        cv2.circle(
                            self.frame,
                            (hand.landmarks[0][0], hand.landmarks[0][1]),
                            radius,
                            color[-1],
                            -1,
                        )
                        for i in range(1, 5):
                            cv2.circle(
                                self.frame,
                                (hand.landmarks[i][0], hand.landmarks[i][1]),
                                radius,
                                color[hand.thumb_state],
                                -1,
                            )
                        for i in range(5, 9):
                            cv2.circle(
                ...
                if self.show_scores:
                    cv2.putText(
                        self.frame,
                        f"Landmark score: {hand.lm_score:.2f}",
        ...
        if self.show_xyz:
            ...
            cv2.rectangle(
                self.frame, (x0, y0), (x0 + 100, y0 + 85), (220, 220, 240), -1
            )
            cv2.putText(
                self.frame,
                f"X:{hand.xyz[0] / 10:3.0f} cm",
                (x0 + 10, y0 + 20),
                cv2.FONT_HERSHEY_PLAIN,
                1,
                (20, 180, 0),
        ...
        if self.show_xyz_zone:
            ...
            cv2.rectangle(
                self.frame,
                tuple(hand.xyz_zone[0:2]),
                tuple(hand.xyz_zone[2:4]),
                (180, 0, 180),
                2,
            )
    ...
    def draw_body(self, body):
        lines = [
            ...
            if body.scores[line[0]] > self.tracker.body_score_thresh
            and body.scores[line[1]] > self.tracker.body_score_thresh
        ]
    ...
    def draw_bag(self, bag):
        if self.show_inferences_status:
            ...
            if bag.get("bpf_inference", 0):
                cv2.rectangle(
                    self.frame, (u, 8 * u), (2 * u, 9 * u), (255, 144, 30), -1
                )
            if bag.get("pd_inference", 0):
                cv2.rectangle(
                    self.frame, (2 * u, 8 * u), (3 * u, 9 * u), (0, 255, 0), -1
                )
        ...
        if body and self.show_body:
            ...
            if focus_zone:
                cv2.rectangle(
                    self.frame,
                    tuple(focus_zone[0:2]),
                    tuple(focus_zone[2:4]),
                    (0, 255, 0),
                    2,
                )
    ...
    def waitKey(self, delay=1):
        if self.show_fps:
            self.tracker.fps.draw(
                self.frame, orig=(50, 50), size=1, color=(240, 180, 100)
            )
